...
const computedSplitBound = () => {
  ...
  let _min = convertToNumber(props.min, total);
  let _max = convertToNumber(props.max, total);
  // 边界校验：确保 min <= max
  if (_min > _max) {
    [_min, _max] = [_max, _min];
  }
  ...
  return Math.max(_min, Math.min(_split, _max));
};
...
const dragEnd = () => {
  ...
  nextTick(() => {
    store.editor && toRaw(store.editor).layout?.();
  });
};
