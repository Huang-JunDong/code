import { AttrsType, MenuElement, ATTR_LIST, SPLIT_SYMBOL, LayoutMenuDirection } from '../type';
import { computeRectPosition } from './getInfo';

export { ATTR_LIST, SPLIT_SYMBOL, LayoutMenuDirection };
/**
 * 阻止默认事件和冒泡
 * @param { Event } e 事件参数
 * @returns { void }
...
export const filterAttrs = (
  ...
  (ATTR_LIST as Array<keyof AttrsType>).forEach((key) => {
    if (options[key]) {
      res[key] = options[key];
    }
    if (params[key]) {
      if (res[key]) {
        res[key] += SPLIT_SYMBOL[key as keyof typeof SPLIT_SYMBOL] + params[key];
      } else {
        res[key] = params[key];
      }
    }
